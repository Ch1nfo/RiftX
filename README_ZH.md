...
RiftX 在 Tsecbench v1 榜单中取得 **93.47/100**，排名 **第 5 名**。本次跑分使用 `deepseek-flash` 模型配置。

![RiftX Tsecbench v1 跑分结果](docs/images/riftx-tsecbench-v1.png)

> 榜单记录：综合得分 `93.47`，排名 `5`。
...
> 总得分 `20800`，综合得分 `100`，完成率 `100%`，完成 `40/40` 道题。

### XBOW Validation Benchmarks

RiftX 也参与了 TSECBENCH 的 XBOW Validation Benchmarks。

![RiftX TSECBENCH 跑分结果](docs/images/riftx-benchmark.jpg)

> 总得分 `27440`，综合得分 `96.96`，完成率 `97.1%`，完成 `101/104` 道题。

## 功能特性

### Agent 工作台

- **流式会话** — 实时呈现文本、Thinking、工具调用、错误和任务状态；终态事件会与持久化 Transcript 对账，使 SSE 间隙中遗漏的回复无需切换会话也能显示。
- **连续对话** — 可在 Agent 运行期间发送补充指令；对话区自动跟随最新内容，也可回到历史位置查看。
- **会话管理** — 会话按工作目录隔离，未发送草稿按会话独立保留；侧栏显示实时运行状态，并将最近活动的运行会话自动置顶。会话同时支持 AI 自动命名、切换、归档、恢复已归档会话和永久删除。
- **上下文管理** — 显示输入、输出、缓存和剩余 Token；接近限制时，RiftX 会在工具回合之间压缩上下文而不中断当前任务，持久化压缩预算以便重启后继续估算，压缩请求失败时自动退避，并从持久化发现、子 Agent 状态和本机输出 Artifact 重建隐藏且有界的调查胶囊。
- **图片与附件** — 单次可附加最多四张图片（每张 8 MB）以及五个受支持的文本/代码文件（每个 2 MB、合计 4 MB），可通过 `Ctrl/Cmd + V` 直接粘贴截图、预览或移除待发内容，并按会话隔离。模型不支持图像时会明确拒绝，发送失败则恢复附件供重试；消息快照使用内容哈希对应的本机图片 URL，不再重复传输 Base64 数据。
- **结果优先** — Agent 在任务结束时直接展示结果，并将生成报告作为可选的下一步，不再为每个任务默认消耗时间和 Token 编写报告。
...
页面快照会生成适合模型读取的文本，并使用 `e1`、`e2` 等稳定元素引用。不同 identity 拥有隔离的 Cookie 与 Storage，可用于并行验证匿名、低权限和高权限状态。启用模型图像输入后，截图可以直接作为视觉上下文发送给模型。浏览器截图也会内联显示在工具卡片中，并可打开全屏查看。
...
每个已连接 Server 都有独立的调用保护：最多同时运行两个调用，单次调用上限 120 秒；连续失败三次后打开 Circuit Breaker，并在 30 秒冷却后重新探测。排队调用支持取消；如果 Server 忽略取消信号，对应并发槽会保留到实际调用结束，避免超时僵尸调用无界累积。
...
- 大型爬取清单会保存为本机输出 Artifact；Agent 只接收有界预览，并可通过 `read` 或 `grep` 查看完整文件。

### 长工具输出

大型 `crawl`、MCP、`web_search` 和 `web_fetch` 结果不再必须完整占用模型上下文。16,000 字符以内的输出继续内联；更长的结果保存到当前会话的本机 Artifact 目录，Agent 只接收有界的头尾预览及完整文件路径。主 Agent 与子 Agent 的 Artifact 相互隔离，并设有单文件大小与保留文件数量上限。
...
- `confirmed` 发现必须包含来自真实工具结果、已捕获请求或截图的可解析硬证据；任务调度或发现记录本身的确认信息不能证明自己的结论。
...
- 发现、保留截图和长输出 Artifact 独立保存，损坏 JSON 会保留原始文件备份，写入采用临时文件加原子替换。
- 会话重新打开或上下文压缩后，调查胶囊会从权威持久化状态重建，而不是不断累积第二套模型生成的记忆。
...
| `DELETE` | `/api/sessions/:id/archive` | 恢复已归档会话 |
| `GET` | `/api/sessions/:id/stream` | 订阅 SSE 会话事件 |
| `GET` | `/api/sessions/:id/messages` | 读取会话消息 |
| `GET` | `/api/sessions/:id/messages/image/:ref` | 读取以内容哈希引用的会话图片 |
