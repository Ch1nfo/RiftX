...
- **长任务连续性** — 对账流式传输中遗漏的消息，在任务运行期间压缩上下文，并从持久化发现、子 Agent 与 Artifact 重建有界的调查胶囊。
...
- **上下文管理** — 显示输入、输出、缓存和剩余 Token；接近限制时，RiftX 可在工具回合之间压缩上下文而不中断当前任务，再从持久化发现、子 Agent 状态和本机输出 Artifact 重建隐藏且有界的调查胶囊。
