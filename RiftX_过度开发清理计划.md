...
| 4 | Replay/Enum 兼容收尾 | completed | 本提交 | 聚焦 `179 passed`；装配 `56 passed`；Artifact/API `94 passed`；migration/Signal `64 passed`；全仓收集 `4370 tests`；Ruff、`compileall`、Release Gate ready | 删除 Audit Artifact 新写入、Web/MCP Code Audit 副作用、Signal factory 和退役 effect 操作，净删 425 行；保留历史 Enum、ORM、migration、Signal、Snapshot 解析与 Safety Stop |
