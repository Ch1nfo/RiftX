"""Trusted activation gate for a cgroup-contained process.

This file intentionally imports only the standard library so it can be
executed by absolute path even when the target environment has a clean or
custom ``PYTHONPATH``.
"""
...
try:
    import fcntl
except ImportError:  # pragma: no cover - launcher is POSIX-only
...
def _verify_membership(path: Path) -> None:
    ...
    if expected not in raw.splitlines():
        raise RuntimeError("launcher pid is absent from target cgroup")


def _report(control_fd: int, payload: bytes) -> None:
    if os.write(control_fd, payload) != len(payload):
        raise OSError("short launcher control write")


def _read_target_environment(fd: int) -> dict[str, str]:
    """Read and validate the inert target environment after activation."""
...
def _ancestor_cgroup_procs(delegated_root: Path) -> tuple[Path, ...]:
    """Resolve the delegated root and every visible cgroup-v2 ancestor."""
...
def _clear_capabilities() -> None:
    """Clear inheritable, permitted, effective, and ambient capabilities."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        identity_values = (args.payload_uid, args.payload_gid)
        ...
        # Parent death or an aborted start closes the socket and produces EOF.
        # Only the exact activation byte authorizes exec; no target code runs
        # before containment identity has been durably admitted upstream.
        activation = os.read(args.control_fd, 1)
        if activation != _ACTIVATE:
            return _LAUNCHER_FAILURE
        ...
        try:
            ...
            _report(args.control_fd, message)
    ...
    try:
        ...
        # A successful exec atomically closes this descriptor.  The parent
        # treats EOF as exec confirmation; on failure we retain it long enough
        # to report an explicit error instead of manufacturing a successful
        # ProcessHandle for a missing executable.
        if fcntl is None:
            raise RuntimeError("POSIX descriptor controls are unavailable")
        ...
        try:
            ...
            pass
        print(f"riftx cgroup launcher exec failed: {exc}", file=sys.stderr, flush=True)
        try:
