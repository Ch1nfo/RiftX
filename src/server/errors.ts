import { BoardError } from "./collaboration/store";
...
function errorStatus(error: unknown, fallback: number) {
  return (error instanceof RiftxError || error instanceof BoardError) ? error.status : fallback;
}
...
export function errorResponse(error: unknown, fallbackMessage: string, fallbackStatus = 500) {
  const payload = error instanceof RiftxError || error instanceof BoardError
