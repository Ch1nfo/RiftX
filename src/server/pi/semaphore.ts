...
/** Fair, abort-aware concurrency limiter shared by independent tool pools. */
export class Semaphore {
  private limit: number;
  private active = 0;
  private readonly waiters: Waiter[] = [];

  constructor(limit: number, private readonly abortMessage: string) {
  ...
  acquire(signal?: AbortSignal, onStart?: () => void) {
    if (signal?.aborted) return Promise.reject(new Error(this.abortMessage));
    return new Promise<() => void>((resolve, reject) => {
      ...
      const onAbort = () => {
        ...
        waiter.settled = true;
        reject(new Error(this.abortMessage));
        this.pump();
      };
  ...
  private pump() {
    while (this.active < this.limit && this.waiters.length) {
      ...
      if (waiter.signal?.aborted) {
        waiter.cleanup();
        waiter.settled = true;
        waiter.reject(new Error(this.abortMessage));
        continue;
      }
      waiter.cleanup();
      waiter.settled = true;
