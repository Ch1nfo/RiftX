      ...
      <div className="settings-grid">
        ...
        <section id="model-agent" className="settings-card">
          ...
          <div className="form-grid">
            ...
            <div className="form-grid-full"><label className="toggle-row"><span><strong>{t("supportsImages")}</strong><small>{t("supportsImagesHint")}</small></span><input type="checkbox" checked={profile.supportsImages === true} onChange={(event) => updateProfile({ supportsImages: event.target.checked })} /></label></div>
          </div>
        </section>
        ...
        <section className="settings-card"><div className="card-heading"><div><h2>{t("browserScope")}</h2><p>{t("browserScopeDesc")}</p></div>{sectionSaveButton("browserScope", () => void saveSection("browserScope", { browserScope: browserScopeDraft.split(/\r?\n/).map((line) => line.trim()).filter(Boolean), browserIgnoreTlsErrors: config.browserIgnoreTlsErrors }))}</div><Field label={t("browserScope")} hint={t("browserScopeHint")}><textarea className="settings-prompt-input" value={browserScopeDraft} onChange={(event) => setBrowserScopeDraft(event.target.value)} placeholder={"10.0.0.0/8\n10.0.181.248:8000\n*.target.com"} rows={5} spellCheck={false} /></Field><label className="toggle-row"><span><strong>{t("ignoreTlsErrors")}</strong><small>{t("ignoreTlsErrorsDesc")}</small></span><input type="checkbox" checked={config.browserIgnoreTlsErrors !== false} onChange={(event) => setConfig({ ...config, browserIgnoreTlsErrors: event.target.checked })} /></label></section>
