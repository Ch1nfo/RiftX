...
export function SettingsPage() {
  ...
  const [mcpServersDraft, setMcpServersDraft] = useState("");
  const [mcpTesting, setMcpTesting] = useState(false);
  const [mcpResults, setMcpResults] = useState<Array<{ name: string; ok: boolean; toolCount?: number; error?: string }> | null>(null);
  ...
  const saveMcpServers = () => {
    ...
    void saveSection("mcpServers", { mcpServers });
  };
  const runMcpTest = async () => {
    ...
    setError("");
    try {
      const response = await fetch("/api/settings/mcp/test", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mcpServers }) });
      const data = await response.json() as { results?: Array<{ name: string; ok: boolean; toolCount?: number; error?: string }>; error?: string };
      if (!response.ok || !Array.isArray(data.results)) throw new Error(data.error ?? t("mcpTestFailed"));
      setMcpResults(data.results);
    } catch (reason) {
      setError(reason instanceof Error && reason.message ? reason.message : t("mcpTestFailed"));
    } finally {
      setMcpTesting(false);
    }
  };
