  ...
  return (agent) => {
    ...
    const browserTool = defineTool<typeof parameters, { action: BrowserAction; url: string }>({
      ...
      promptGuidelines: [
        ...
        "An out-of-scope navigate pauses for user approval and resumes the same call automatically; do not retry solely because an approval prompt appeared or fall back to curl for interactive validation.",
