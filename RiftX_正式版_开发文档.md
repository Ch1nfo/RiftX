...
“超过通用 Agent”是产品长期追求的方向，不纳入正式版的评测目标或发布验收条件。渗透测试和代码审计的真实能力受到目标类型、操作者经验、可用工具、任务周期、上下文积累和组织知识等多种因素影响，不强制将该追求压缩成单一分数、排行榜或固定提升比例。
...
本阶段的目的，是建立稳定的研发反馈系统：发现能力短板、复现失败、检查版本回归、评估 Capability Pack 的实际影响，并为安全发布提供证据。
