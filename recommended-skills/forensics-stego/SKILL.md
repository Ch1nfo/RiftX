...
3. 压缩包:伪加密/注释字段先看;密码爆破走 `zip2john` → hashcat + 自带字典。
...
镜像里没有内存取证套件:先 `strings` 抽凭据/flag 模式,再 python 手工解析结构
(题给的 profile/符号常是下一步提示)。

## RiftX 工作流

- 切出的文件统一落 work/carved/,按来源命名;flag 一出现立即
  `benchmark_control(action="submit", uniqueCode, flag)`。
- 关键结论(载体类型、隐写方式、破解出的密码)`benchmark_control(action="checkpoint")`
  (signalKind=note,evidenceRef=文件名+提取命令);破解出的密码可能可复用,记黑板。
- 大 pcap/镜像扫描放后台、输出重定向到文件再 grep,不刷上下文;每次 attempt 限时
  30 分钟,到点前 checkpoint 留进度。
