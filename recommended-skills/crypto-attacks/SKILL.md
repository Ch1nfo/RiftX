...
- 推导逐步写进 work/solve.py(可重放),还原出的中间量(n、d、密钥、明文片段)
  `benchmark_control(action="checkpoint")` 留黑板(signalKind=note,
  evidenceRef=中间值来源);flag 立即 `benchmark_control(action="submit", uniqueCode, flag)`。
- 先估算可行性再动手:数学题的"便宜口子"几乎总在题面或参数本身。
- 每次 attempt 限时 30 分钟;在线爆破受每题 120 秒枚举预算约束(单次调用 30 秒,
  自写脚本记得设 passwordEnumeration=true)。
