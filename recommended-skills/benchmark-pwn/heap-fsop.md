# Benchmark Pwn - Heap FILE Structure Attacks
...
## Table of Contents
- [Fastbin stdout Vtable Two-Stage Hijack for PIE + Full RELRO](#fastbin-stdout-vtable-two-stage-hijack-for-pie--full-relro)
- [_IO_buf_base Null Byte Overwrite for stdin Hijack](#_io_buf_base-null-byte-overwrite-for-stdin-hijack)
- [glibc 2.24+ _IO_FILE Vtable Validation Bypass](#glibc-224-_io_file-vtable-validation-bypass)
- [Unsorted Bin Attack on stdin _IO_buf_end](#unsorted-bin-attack-on-stdin-_io_buf_end)
- [Unsorted Bin Corruption via mp_ Structure](#unsorted-bin-corruption-via-mp_-structure)
- [realloc(ptr, 0) as free() for UAF](#reallocptr-0-as-free-for-uaf)
- [Single-Byte Reference Counter Wraparound to UAF](#single-byte-reference-counter-wraparound-to-uaf)
