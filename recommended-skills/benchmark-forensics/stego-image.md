# Benchmark Forensics - Image Steganography
...
**Key insight:** Edge-matching works by minimizing pixel differences at shared borders. The greedy approach (place piece with smallest total edge difference to already-placed neighbors) works well for most benchmark puzzles. For harder puzzles, add backtracking.
...
**Key insight:** Benchmark jigsaw challenges rarely require manual work. Carve pieces, stitch, run `gaps` — it uses a genetic algorithm to reassemble in minutes. Read `exiftool` on each piece for the size hint.
