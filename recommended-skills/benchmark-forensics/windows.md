...
**Key insight:** ADS are invisible to `dir` (without `/r`), Explorer, and most forensic tools that only check default data streams. The Sleuth Kit's `fls` with the colon notation (`inode-type-id`) is the most reliable way to enumerate and extract ADS from images. Malware uses ADS to hide payloads; benchmark challenges use them to hide flags. The `Zone.Identifier` stream is the most common ADS — it's automatically added by browsers and email clients to downloaded files.
...
**Key insight:** Before spending hours carving LSASS or walking process heaps, run `clipboard` — benchmark challenges about a user who copied their password to the clipboard always surface here. Combine with `cmdline`, `consoles`, and `filescan` for full user-activity reconstruction.
