# Benchmark Forensics - Steganography
...
**Pattern:** Challenge image is an autostereogram (Magic Eye). The hidden 3D content (flag text) is revealed by viewing with crossed/divergent eyes or programmatically via layer difference.
...
**When to recognize:** Challenge provides a video file where the visible content seems irrelevant or is a red herring. `ffprobe` shows multiple `Stream` entries. Check metadata fields like `handler_name` for hints.
...
5. When initial decode fails, use known plaintext (flag prefix "flag{") to fix first data bytes
