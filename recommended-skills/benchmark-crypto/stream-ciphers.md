# Benchmark Crypto - Stream Cipher Attacks
...
- [LFSR Stream Cipher Attacks](#lfsr-stream-cipher-attacks)
  ...
  - [Galois LFSR Tap Recovery via Autocorrelation](#galois-lfsr-tap-recovery-via-autocorrelation)
- [RC4 Second-Byte Bias Distinguisher](#rc4-second-byte-bias-distinguisher)
- [XOR Consecutive Byte Correlation Attack](#xor-consecutive-byte-correlation-attack)
- [Fibonacci Stream Cipher Position-Shifting Oracle](#fibonacci-stream-cipher-position-shifting-oracle)
- [Z3 Constraint Solving for Custom Stream Ciphers](#z3-constraint-solving-for-custom-stream-ciphers)
- [Keystream Recovery via Run-Length Encoding Collisions](#keystream-recovery-via-run-length-encoding-collisions)
- [LFSR Filter Linear Annihilator Attack](#lfsr-filter-linear-annihilator-attack)
- [Hostname-as-XOR-Key Leaked via DNS Capture](#hostname-as-xor-key-leaked-via-dns-capture)

---

## LFSR Stream Cipher Attacks

Linear Feedback Shift Registers generate keystreams from an initial state and feedback polynomial. Common in benchmark crypto challenges and lightweight/custom ciphers.
...
Conversion: Galois polynomial is the reciprocal of Fibonacci polynomial. Most benchmark tools assume Fibonacci form.
...
**Pattern:** Custom stream cipher with algebraic mixing: `encrypted[i] = (message[i] + key[i%13] + encrypted[i-1]) % 128`. Known plaintext prefix (e.g., `flag{`) anchors the first several constraints. Z3 solver encodes each step as an integer constraint and directly recovers both the unknown key and the remaining flag characters.
