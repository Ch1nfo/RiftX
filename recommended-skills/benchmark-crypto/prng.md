# Benchmark Crypto - PRNG & Key Recovery

Foundational PRNG state/seed recovery techniques. For advanced attacks (MT19937 constraint propagation, Rule 86 cellular automaton, Java LCG MITM, LFSR bit-fold, Z3 timing oracle, randcrack DSA, NTP-poisoned UUID), see [prng-attacks.md](prng-attacks.md).

## Table of Contents
- [Mersenne Twister (MT19937) State Recovery](#mersenne-twister-mt19937-state-recovery)
- [MT State Recovery from random.random() Floats via GF(2) Matrix](#mt-state-recovery-from-randomrandom-floats-via-gf2-matrix)
...
- [GF(2) Matrix PRNG Seed Recovery](#gf2-matrix-prng-seed-recovery)
- [Middle-Square PRNG Brute Force](#middle-square-prng-brute-force)
- [Deterministic RNG from Flag Bytes + Hill Climbing](#deterministic-rng-from-flag-bytes--hill-climbing)
- [Byte-by-Byte Oracle with Random Mode Matching](#byte-by-byte-oracle-with-random-mode-matching)
- [RSA Key Reuse / Replay](#rsa-key-reuse--replay)
- [Password Cracking Strategy](#password-cracking-strategy)
- [Logistic Map / Chaotic PRNG Seed Recovery](#logistic-map--chaotic-prng-seed-recovery)
- [V8 XorShift128+ State Recovery (Math.random Prediction)](#v8-xorshift128-state-recovery-mathrandom-prediction)

See [prng-attacks.md](prng-attacks.md) for advanced attacks (2017+).
