# Benchmark Crypto - PRNG Attacks

Advanced challenge-specific PRNG attacks from 2017 onward. For foundational PRNG recovery (MT19937, LCG parameter recovery, ChaCha20, V8 XorShift128+, password cracking), see [prng.md](prng.md).

## Table of Contents
- [Mersenne Twister Seed Recovery from Subset Sum](#mersenne-twister-seed-recovery-from-subset-sum)
- [MT19937 State Recovery via Constraint Propagation](#mt19937-state-recovery-via-constraint-propagation)
- [Rule 86 Cellular Automaton PRNG Reversal via Z3](#rule-86-cellular-automaton-prng-reversal-via-z3)
- [Java LCG Seed Meet-in-the-Middle via Partial Modulo](#java-lcg-seed-meet-in-the-middle-via-partial-modulo)
- [LCG Backward Stepping via Multiplicative Inverse](#lcg-backward-stepping-via-multiplicative-inverse)
- [LFSR Bit-Fold Recovery from ASCII Parity](#lfsr-bit-fold-recovery-from-ascii-parity)
- [Z3 Solve-Time Timing Oracle on PRNG](#z3-solve-time-timing-oracle-on-prng)
- [randcrack-Fed DSA k Prediction](#randcrack-fed-dsa-k-prediction)
- [Time-Seeded PRNG Offset via Format-String Global Write](#time-seeded-prng-offset-via-format-string-global-write)
- [NTP-Poisoned PRNG State Leak via UUID XOR](#ntp-poisoned-prng-state-leak-via-uuid-xor)
