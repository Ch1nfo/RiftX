# Benchmark Crypto - RSA Attacks (Part 2: Specialized Techniques)

## Table of Contents
- [RSA p=q Validation Bypass](#rsa-pq-validation-bypass)
- [RSA Cube Root CRT when gcd(e, phi) > 1](#rsa-cube-root-crt-when-gcde-phi--1)
- [Factoring n from Multiple of phi(n)](#factoring-n-from-multiple-of-phin)
- [RSA Signature Forgery via Multiplicative Homomorphism](#rsa-signature-forgery-via-multiplicative-homomorphism)
- [Weak RSA Key Generation via Base Representation](#weak-rsa-key-generation-via-base-representation)
- [RSA with gcd(e, phi(n)) > 1](#rsa-with-gcde-phin--1)
- [Batch GCD for Shared Prime Factoring](#batch-gcd-for-shared-prime-factoring)
- [RSA Partial Key Recovery from dp dq qinv](#rsa-partial-key-recovery-from-dp-dq-qinv)
- [RSA-CRT Fault Attack / Bit-Flip Recovery](#rsa-crt-fault-attack--bit-flip-recovery)
- [RSA Homomorphic Decryption Oracle Bypass](#rsa-homomorphic-decryption-oracle-bypass)
- [RSA with Small Prime Factors and CRT Decomposition](#rsa-with-small-prime-factors-and-crt-decomposition)
- [RSA Timing Attack on Montgomery Reduction](#rsa-timing-attack-on-montgomery-reduction)
- [Bleichenbacher Low-Exponent RSA Signature Forgery](#bleichenbacher-low-exponent-rsa-signature-forgery)
- [Coppersmith Small Roots for Linearly Related Primes](#coppersmith-small-roots-for-linearly-related-primes)
- [ROCA Attack on RSA CVE-2017-15361](#roca-attack-on-rsa-cve-2017-15361)
- [RSA Signature Bypass with e=1 and Crafted Modulus](#rsa-signature-bypass-with-e1-and-crafted-modulus)
- [Dependent-Prime RSA: q = e^-1 mod p](#dependent-prime-rsa-q--e-1-mod-p)
- [RSA Three-Key Pairwise GCD Triangle](#rsa-three-key-pairwise-gcd-triangle)
- [RSA n = p^2*q Schmidt-Samoa Variant](#rsa-n--p2q-schmidt-samoa-variant)
- [Modulus Recovery via GCD of Encryption Residuals](#modulus-recovery-via-gcd-of-encryption-residuals)
- [Textbook RSA Negation via encrypt(-1)](#textbook-rsa-negation-via-encrypt-1)
- [Poly-Exponent RSA: GCD of p^p Combinations](#poly-exponent-rsa-gcd-of-pp-combinations)
- [Biased LSB Oracle with Mode-of-Runs Recovery](#biased-lsb-oracle-with-mode-of-runs-recovery)
- [Cube-Root Wraparound via AES-CTR Length Hint](#cube-root-wraparound-via-aes-ctr-length-hint)
- [RSA p = next_prime(2^k + small) Shared-Prime Batch GCD](#rsa-p--next_prime2k--small-shared-prime-batch-gcd)
- [PNG Encryption Bounded by 512-bit Key → Trailer Replacement](#png-encryption-bounded-by-512-bit-key--trailer-replacement)
- [Modulus Recovery via Plaintext Malleability](#modulus-recovery-via-plaintext-malleability)
- [RSA CRT d_p NULL-Byte Overflow Primes Leak](#rsa-crt-d_p-null-byte-overflow-primes-leak)
- [Textbook RSA Signature Blinding via Message Factoring](#textbook-rsa-signature-blinding-via-message-factoring)
- [Last-Byte Modulus Overwrite via strlen-1 Null Truncation](#last-byte-modulus-overwrite-via-strlen-1-null-truncation)
- [CRC32 Collision Oracle + RSA Homomorphic Signature Forgery](#crc32-collision-oracle--rsa-homomorphic-signature-forgery)
