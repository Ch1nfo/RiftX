...
2. **文档端点用浏览器确认**：`/swagger` `/openapi.json` `/api-docs` `/graphql` `/graphiql`——命中即拿到全量接口定义（本身若未鉴权也是一条暴露面，`benchmark_control` checkpoint 记录，signalKind: new_surface）
3. **重放与篡改在 bash**：`cookies_export` 导出会话 → curl 重放修改（换方法/换路径/加字段/改 token）；GraphQL 在浏览器 `evaluate` 里发 query 最方便（页面上下文自带 token 与同源策略）
4. **并行**：参数/端点 fuzz 用 bash 后台（`nohup` + ffuf/自写脚本）跑，主会话手工做逻辑类测试（越权、mass assignment——交叉 `exploit-authz`，read ~/.riftx/skills/exploit-authz/SKILL.md）
...
- **JWT**：解码看 alg/claims → `alg: none` 与弱密钥（python3 PyJWT 解码/伪造 + `hashcat -m 16500` 配 `/opt/wordlists` 密码字典离线爆破）、claim 篡改（sub/role/exp）重放
...
- 登录/验证码/敏感操作端点连续请求观察限流（无限流=暴力破解面，checkpoint 记录）；在线密码枚举每题累计 120 秒、单次 30 秒（harness 强制），自写爆破脚本记得设 `passwordEnumeration=true`
...
- [ ] 限流验证
- [ ] 每个确认点：`request_detail` 证据 + `benchmark_control` checkpoint

---

## 相关 Skills

`exploit-authz`（越权/BOLA 深入打法）、`exploit-oauth`（OAuth 流程与 JWT 深度）、`recon-dir-scan`（端点发现）、`exploit-sqli`（注入类 API 参数）、`results-storage`（checkpoint 与 work/ 持久化机制）
