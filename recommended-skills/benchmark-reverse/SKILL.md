...
description: Provides reverse engineering techniques for benchmark challenges. Use when the main job is to understand how a compiled, obfuscated, packed, or virtualized target works before exploiting or solving it, including binaries, APKs, WASM, firmware, custom VMs, bytecode, game clients, malware-like loaders, and anti-debug or anti-analysis logic. Do not use it when the vulnerability is already understood and the remaining task is exploitation; use pwn instead. Do not use it for pure web workflows, log or disk forensics, or standalone crypto problems unless reversing the implementation is the real blocker. 中文挑战关键词：逆向 反编译 加壳 脱壳 混淆 虚拟机 分析 二进制 APK 固件 调试 反汇编 补丁 破解 校验
license: MIT
...
- [field-notes.md](field-notes.md) - Quick reference notes: binary types, anti-debugging bypass, specialized patterns, competition case notes

---

## When to Pivot

- If you already understand the binary and now need heap, ROP, or kernel exploitation, switch to `benchmark-pwn`.
- If the challenge is really about recovering deleted files, PCAP data, or disk artifacts, switch to `benchmark-forensics`.
- If the target is a web app and you are only reversing a small client-side helper script, switch to `benchmark-web`.
- If the binary implements a machine learning model and the challenge is about model attacks or adversarial inputs, switch to `benchmark-ai-ml`.
- If the reversed binary's core logic is a cryptographic algorithm or math problem, switch to `benchmark-crypto`.
- If the challenge is a toy VM, encoding puzzle, or pyjail rather than a real binary, switch to `benchmark-misc`.
...
- Case notes: modern competition-specific reversing patterns and older classic challenge patterns
