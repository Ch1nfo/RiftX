# Benchmark Reverse - Platform-Specific Reversing
...
**Benchmark relevance:** Patched binaries need re-signing to run on macOS. Ad-hoc signing (`-s -`) works for local testing.
...
**RISC-V:** See main [tools.md](tools.md) for Capstone disassembly and [platforms-hardware.md](platforms-hardware.md#risc-v-advanced) for advanced extensions and debugging.
...
**Common kernel module challenge patterns:**
```c
...
```text
For benchmark challenges involving game anti-cheat:
...
2. In benchmarks: usually need to bypass specific check, not full anti-cheat
...
# UDS (Unified Diagnostic Services) — common in automotive benchmark challenges
...
**Key insight:** IL2CPP looks opaque but the metadata file carries every type name, method name, and string literal. That is enough to reverse challenge-level logic without ever touching native disassembly.
