# Benchmark Reverse - Tools Reference
...
- [.NET Analysis](#net-analysis)
  - [Tools](#tools)
  - [Two-Stage XOR + AES-CBC Decode Pattern](#two-stage-xor--aes-cbc-decode-pattern)
  - [NativeAOT](#nativeaot)
...
- [boolector SMT2 for Custom Hash Reversal](#boolector-smt2-for-custom-hash-reversal)
...
**Key insight:** When `RijndaelManaged` appears in .NET decompilation, check if Key and IV are set to the same value — this is a common benchmark pattern. The XOR stage often serves as a simple obfuscation layer before the real crypto.
