---
name: benchmark-web
description: Provides web exploitation techniques for benchmark challenges. Use when the target is primarily an HTTP application, API, browser client, template engine, identity flow, or smart-contract frontend/backend surface, including XSS, SQLi, SSTI, SSRF, XXE, JWT, auth bypass, file upload, request smuggling, OAuth/OIDC, SAML, prototype pollution, and similar web bugs. Do not use it for native binary memory corruption, reverse engineering of standalone executables, disk or memory forensics, or pure cryptanalysis unless the web flaw is still the main path to the flag. 中文挑战关键词：注入 跨站 脚本 模板 SSTI SSRF XXE 文件上传 越权 身份认证 登录 会话 JWT OAuth 反序列化 原型污染 请求走私 前端 后端 接口 管理员 凭证 网页应用漏洞利用
license: MIT
...
- If the target is a native binary, custom VM, or firmware image, switch to `benchmark-reverse` first.
- If the HTTP bug only gives you code execution and the hard part becomes memory corruption or seccomp escape, switch to `benchmark-pwn`.
- If the "web" challenge really turns on JWT math, custom MACs, or crypto primitives, switch to `benchmark-crypto`.
- If the web challenge involves analyzing logs, PCAPs, or recovering artifacts from a web server, switch to `benchmark-forensics`.
...
- CVE-shaped playbooks and older challenge patterns that still show up in modern benchmarks
