# Benchmark Web - CVEs & Browser Vulnerabilities
...
- [Broken Auth via Truthy Hash Check](#broken-auth-via-truthy-hash-check)
- [AAEncode/JJEncode JS Deobfuscation](#aaencodejjencode-js-deobfuscation)
- [Protocol Multiplexing — SSH+HTTP on Same Port](#protocol-multiplexing--sshhttp-on-same-port)
...
- [CVE-2024-45409: Ruby-SAML XPath Digest Smuggling](#cve-2024-45409-ruby-saml-xpath-digest-smuggling)
- [CVE-2023-27350: PaperCut NG Authentication Bypass + RCE](#cve-2023-27350-papercut-ng-authentication-bypass--rce)
- [CVE-2024-22120: Zabbix Time-Based Blind SQLi](#cve-2024-22120-zabbix-time-based-blind-sqli)
- [CVE-2012-0053: Apache HttpOnly Cookie Leak via 400 Bad Request](#cve-2012-0053-apache-httponly-cookie-leak-via-400-bad-request)
- [CVE-2014-9734: WordPress RevSlider Upload + MySQL load_file() SSH Pivot](#cve-2014-9734-wordpress-revslider-upload--mysql-load_file-ssh-pivot)
...
See benchmark-misc/pyjails.md for PYTHONWARNINGS technique details.
...
See [server-side-advanced.md](server-side-advanced.md) for full exploit code.
...
See [server-side-advanced-4.md](server-side-advanced-4.md) for full exploit chain.
...
**Key insight:** RCE via plugin upload is rarely the end — extract `wp-config.php` DB creds, connect to the database, and `load_file()` to read any world-readable file (private SSH keys under `/backup/`, `/root/.ssh/`, `/home/*/.ssh/`, CI secrets, etc.), then SSH pivot to the real target box. The WP user almost always has FILE privilege on challenge setups. Exploits to chain with a single file-upload: `wp_revslider_upload_execute`, `wp_admin_shell_upload`, `wp_asset_manager_upload_exec`, `wp_symposium_shell_upload`.
