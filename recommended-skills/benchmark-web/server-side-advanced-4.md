...
- [MongoDB Regex Injection / $where Blind Oracle](#mongodb-regex-injection--where-blind-oracle)
- [Pongo2 / Go Template Injection via Path Traversal](#pongo2--go-template-injection-via-path-traversal)
...
- [wget CRLF Injection for SSRF-to-SMTP](#wget-crlf-injection-for-ssrf-to-smtp)
- [Gopher SSRF to MySQL Blind SQLi](#gopher-ssrf-to-mysql-blind-sqli)
...
- [AMQP/TLS Interception via sslsplit + arpspoof](#amqptls-interception-via-sslsplit--arpspoof)
- [CairoSVG XXE via Oversized width=](#cairosvg-xxe-via-oversized-width)
- [Bazaar (.bzr) Repository Reconstruction via bzr check Loop](#bazaar-bzr-repository-reconstruction-via-bzr-check-loop)
...
Upload, download the PNG, and read the flag off the image (eyeball or OCR). When hunting the flag path, dump `/proc/self/status` first to find the PID, then probe `/proc/<pid>/cwd/flag.txt`, `/proc/<pid>/cmdline`, and `/proc/<pid>/environ`. If the first pass clips (e.g. width=3000), re-render wider — some targets only render the flag at `width="3000"` when the target path is short — re-render wider until the flag path fits.
...
**Key insight:** Exposed `.bzr/` (or `.git/`, `.hg/`, `.svn/`) directories leak full commit history; bzr is particularly friendly because it tolerates partial repos and reports the missing path verbatim, so a wget-in-a-loop solver finishes the job. Always diff revisions, not just `HEAD` — flags, wallet keys, and decryption keys are often *removed* in a later commit but still recoverable. Once the tree is reconstructed you can chain with history-split secrets, where one revision stores a base64 ciphertext and a later revision stores the AES-ECB key.
