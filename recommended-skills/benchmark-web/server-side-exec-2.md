# Benchmark Web - Server-Side Code Execution & Access Attacks (Part 2)

## Table of Contents
- [SQLi Keyword Fragmentation Bypass](#sqli-keyword-fragmentation-bypass)
- [SQL WHERE Bypass via ORDER BY CASE](#sql-where-bypass-via-order-by-case)
- [SQL Injection via DNS Records](#sql-injection-via-dns-records)
- [Bash Brace Expansion for Space-Free Command Injection](#bash-brace-expansion-for-space-free-command-injection)
- [Common Lisp Injection via Reader Macro](#common-lisp-injection-via-reader-macro)
- [PHP7 OPcache Binary Webshell + LD_PRELOAD disable_functions Bypass](#php7-opcache-binary-webshell--ld_preload-disable_functions-bypass)
- [Wget GET Parameter Filename Trick for PHP Shell Upload](#wget-get-parameter-filename-trick-for-php-shell-upload)
- [Tar Filename Command Injection](#tar-filename-command-injection)
- [PNG/PHP Polyglot Upload + Double Extension + disable_functions Bypass](#pngphp-polyglot-upload--double-extension--disable_functions-bypass)
- [PHP BMP Pixel Webshell with Filename Truncation](#php-bmp-pixel-webshell-with-filename-truncation)
- [Editor Backup File Source Disclosure](#editor-backup-file-source-disclosure)
- [date -f Arbitrary File Read](#date--f-arbitrary-file-read)
- [Apache mod_rewrite PATH_INFO Bypass](#apache-mod_rewrite-path_info-bypass)
- [PHP ReDoS to Skip Code Execution](#php-redos-to-skip-code-execution)
- [Custom Serializer Integer Overflow 256 to 0 Length](#custom-serializer-integer-overflow-256-to-0-length)
- [Pickle Chaining via STOP Opcode Stripping](#pickle-chaining-via-stop-opcode-stripping) *(stub — see [server-side-deser.md](server-side-deser.md))*
...
- [Unanchored Regex Command Injection](#unanchored-regex-command-injection)
- [Jinja2 SSTI via globals.__self__.exec() String Concat Bypass](#jinja2-ssti-via-globals__self__exec-string-concat-bypass)
- [web.py reparam() eval + __subclasses__ with Blanked Builtins](#webpy-reparam-eval--__subclasses__-with-blanked-builtins)
- [Redis Lua Injection via redis.call()](#redis-lua-injection-via-rediscall)
- [PHP create_function String Interpolation RCE](#php-create_function-string-interpolation-rce)
- [php://input + NULL Byte + ~Bitwise base64 Filter Bypass](#phpinput--null-byte--bitwise-base64-filter-bypass)
- [EXIF ImageDescription Shell Injection via exiftool](#exif-imagedescription-shell-injection-via-exiftool)
- [.phar Extension Bypass for PHP Upload Blacklists](#phar-extension-bypass-for-php-upload-blacklists)
- [vsftpd 2.3.4 Smiley-Face Backdoor](#vsftpd-234-smiley-face-backdoor)
...
Strip pickle STOP opcode (`\x2e`) from first payload, concatenate second — both `__reduce__` calls execute in single `pickle.loads()`. Chain `os.dup2()` for socket output. See [server-side-deser.md](server-side-deser.md) for full exploit code.
