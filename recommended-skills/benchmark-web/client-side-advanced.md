# Benchmark Web - Advanced Client-Side Attacks
...
- [CSS Font Glyph Width + Container Query Exfiltration](#css-font-glyph-width--container-query-exfiltration)
- [Hyperscript CDN CSP Bypass](#hyperscript-cdn-csp-bypass)
- [PBKDF2 Prefix Timing Oracle via postMessage](#pbkdf2-prefix-timing-oracle-via-postmessage)
- [Client-Side HMAC Bypass via Leaked JS Secret](#client-side-hmac-bypass-via-leaked-js-secret)
- [Terminal Control Character Obfuscation](#terminal-control-character-obfuscation)
- [CSP Bypass via Cloud Function Whitelisted Domain](#csp-bypass-via-cloud-function-whitelisted-domain)
- [CSP Nonce Bypass via base Tag Hijacking](#csp-nonce-bypass-via-base-tag-hijacking)
- [XSSI via JSONP Callback with Cloud Function Exfiltration](#xssi-via-jsonp-callback-with-cloud-function-exfiltration)
- [CSP Bypass via link prefetch](#csp-bypass-via-link-prefetch)
- [Cross-Origin XSS via Shared Parent Domain Cookie Injection](#cross-origin-xss-via-shared-parent-domain-cookie-injection)
- [Chrome Unicode URL Normalization Bypass](#chrome-unicode-url-normalization-bypass)
- [XSS Dot-Filter Bypass via Decimal IP and Bracket Notation](#xss-dot-filter-bypass-via-decimal-ip-and-bracket-notation)
- [XSS via Referer Header Injection](#xss-via-referer-header-injection)
- [Java hashCode() Collision for Auth Bypass](#java-hashcode-collision-for-auth-bypass)
- [CSS @font-face unicode-range Data Exfiltration](#css-font-face-unicode-range-data-exfiltration)
- [postMessage Null Origin Bypass via data URI Iframe](#postmessage-null-origin-bypass-via-data-uri-iframe)
- [CSP Bypass via Attacker-Controlled Mime Type for Same-Origin Scripts](#csp-bypass-via-attacker-controlled-mime-type-for-same-origin-scripts)
- [React Component State Extraction via __reactInternalInstance$](#react-component-state-extraction-via-__reactinternalinstance)
- [CloudFlare Cache Poisoning via .js Username + Stored Self-XSS](#cloudflare-cache-poisoning-via-js-username--stored-self-xss)
