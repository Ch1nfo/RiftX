# Benchmark Web - Advanced Server-Side Techniques (Part 3)

CVE-era and older advanced server-side techniques. For parts 1-2, see [server-side-advanced.md](server-side-advanced.md) and [server-side-advanced-2.md](server-side-advanced-2.md).

## Table of Contents
- [WAV Polyglot Upload Bypass via .wave Extension](#wav-polyglot-upload-bypass-via-wave-extension)
- [Multi-Slash URL Parser `path.startswith` Bypass](#multi-slash-url-parser-pathstartswith-bypass)
- [Xalan XSLT math:random() Seed Guess](#xalan-xslt-mathrandom-seed-guess)
- [SoapClient _user_agent CRLF Method Smuggling](#soapclient-_user_agent-crlf-method-smuggling)
- [`gopher://` No-Host URL Scheme Bypass]
- [SSRF Credential Leak via Attacker-Specified Outbound URL](#ssrf-credential-leak-via-attacker-specified-outbound-url)
