...
- `environment/add` — experimental; add or replace a named remote environment by `environmentId` and `execServerUrl` for later selection by `thread/start` or `turn/start`; optional `connectTimeoutMs` overrides the WebSocket connection timeout, and optional sensitive `bootstrapToken` is sent only as the initial WebSocket bearer credential; returns `{}` and does not change the default environment.
...
- `skills/extraRoots/set` — replace the app-server process runtime extra standalone skill roots. The roots are not persisted; missing directories are accepted and simply load no skills. Experimental clients may set `exclusive: true` to exclude configured, project, plugin, and bundled roots.
...
- `commandExecution` — `{id, command, cwd, status, commandActions, aggregatedOutput?, stdout?, stderr?, exitCode?, durationMs?}` for sandboxed commands; `status` is `inProgress`, `completed`, `failed`, or `declined`. Completed live items include separate `stdout` and `stderr` when the runtime provides them.
...
- `item/commandExecution/outputDelta` — streams command output with `stream` set to `stdout` or `stderr`; append deltas in order to render live output. Fast commands may complete without deltas, so consumers that require complete output should fall back to the final item's `stdout` and `stderr`.
